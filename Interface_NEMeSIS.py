...
class DataInputPage:
    def __init__(self):
        self.content = pn.Column("# Data Input Selection", desc_str.input_selection_string,
                                data_input_page)
    def view(self):
        return self.content


class DataAlignmentPage:
    def __init__(self):
        self.content = pn.Column("# Data Alignment Section", desc_str.alignment_description_string,
                                alignment_page)
    def view(self):
        return self.content
...
# Decision page between data inputs
to_data_alignment_button = pn.widgets.Button(name='Go To Data Alignment (mzML spectra or m/z peak lists per sample inputs)',
                                             button_type='primary', disabled=False, height=100)
to_matrix_reading_button = pn.widgets.Button(name='Go To 2D Matrix Data Reading (aligned data table input)',
                                             button_type='primary', disabled=False, height=100)

data_input_page = pn.Row(to_data_alignment_button, to_matrix_reading_button)


# mzML Raw Spectra Conversion and Data Alignment Page
...
class ReadingMZML_Files(param.Parameterized):
    ...
    def _confirm_spectral_conv_button(self, event):
        ...
        for f in filename_alignment.value:
            # Files
    ...
    def _save_conversion_res_button(self, event):
        ...
        # Save Word
        document.save(path_dl + f'/{conversion_download_name.value}.docx')
...
'min_spacing' is the smaller of the two spacings from the peak apex to its two neighboring points."""),
        ...
        widgets2 = {
            'save_conversion_res_button': pn.widgets.Button(
                name="Save parameters and description (downloads folder) and the Excel with the samples' peak lists (NEMeSIS FilesToAlign folder). Progress to Data Alignment",
...
class DataAlignmentObject(param.Parameterized):
    ...
    def _confirm_button_alignment(self, event):
        ...
        try:
            ...
            alignment_page.append(move_to_matrix_reading_button)
...
# Needed Widgets
filename_alignment = pn.widgets.FileSelector(directory=os.getcwd()+ '/Files_To_Align', name='Select Desired Files to read')

confirm_button_filename_alignment = pn.widgets.Button(name='Read Samples', button_type='primary', disabled=False)


def _confirm_button_filename_alignment(event):
    ...
    n_mzML_files = 0
    for i in filename_alignment.value:
        if i.endswith('mzML'):
            n_mzML_files += 1

    # No Files Provided
    if len(filename_alignment.value) == 0:
    ...
    # One Excel file is provided or multiple csv files (no mzML files) - Data Alignment
        ...
        # A single Excel File
        if len(filename_alignment.value) == 1:
            file = filename_alignment.value[0]
            if file.endswith('.xlsx') or file.endswith('xls'):
                try:
                    ...
                    alignment_storage.update_widgets(filename_alignment.value)
                    alignment_page.append(data_alignment_section)
                    spectra_store.show_spectra = False
                    move_to_matrix_reading_button.disabled = True
        ...
        # Multiple csv files
            ...
            # All csv files
            if n_csv_files == len(filename_alignment.value):
                try:
                    full_data = []
                    for file in filename_alignment.value:
                        # Reading the file
    ...
    # Multiple files provided
        ...
        # All mzML files
        if n_mzML_files == len(filename_alignment.value):
            # Updating layout
            while len(alignment_page)>2:
                alignment_page.pop(-1)
            alignment_page.append(data_mzML_section)
            mzML_store.files = filename_alignment.value
            spectra_store.show_spectra = True
            data_reading_section.append(
                            f'**{len(filename_alignment.value)} mzML files** were detected.')
            move_to_matrix_reading_button.disabled = True
...
confirm_button_filename_alignment.on_click(_confirm_button_filename_alignment)
...
dataset_download_name = pn.widgets.TextAreaInput(name='Name to save the dataset as:',
                                           ...
                                           description='This file will be saved in the NEMeSIS folder so that it can be immediately read in the next section.')

# Button to save files
save_alignment_files_button = pn.widgets.Button(name='Save Aligned Dataset (NEMeSIS folder) and Alignment Description (Downloads folder) and Go To Matrix Data Reading',
                                                button_type='success', disabled=False, icon=iaf.download_icon)
...
def _save_alignment_files_button(event):
    ...
    try:
        ...
        move_to_matrix_reading_button.disabled = False
...
class Visualizing_Spectra(param.Parameterized):
    ...
    def _confirm_spectra_button(self, event):
        ...
        # Create figure
        fig, axs = plt.subplots(len(self.see_spec), 3, figsize=(4,3), constrained_layout=True)
        #plt.rc('axes', labelsize=3, titlesize=4)
        ...
        for (s, s1) in zip(self.see_spec, range(len(self.see_spec))):
            ...
            for (sf, sf1) in zip(spec_form.keys(), range(len(spec_form))):
                ...
                if len(self.see_spec) == 1:
                    axs[sf1].plot(spec_form[sf]['masses'], spec_form[sf]['ints'], linewidth=0.2)
                    axs[sf1].set_xlim(self.mass_lim)
                    axs[sf1].set_title(s+' - ' + sf, size=4)
                    axs[sf1].tick_params(axis='both', which='major', labelsize=3)

                # If multiple sample selected
                else:
                    axs[s1, sf1].plot(spec_form[sf]['masses'], spec_form[sf]['ints'], linewidth=0.2)
                    axs[s1, sf1].set_xlim(self.mass_lim)
                    axs[s1, sf1].set_title(s+' - ' + sf, size=4)
                    axs[s1, sf1].tick_params(axis='both', which='major', labelsize=3)
...
move_to_matrix_reading_button = pn.widgets.Button(name='Go To 2D Matrix Data Reading and clear this section (must save aligned dataset to become pressable)',
                                             button_type='primary', disabled=True, height=100)

def _move_to_matrix_reading_button(event):
    "Moves analysis to the main sequence of NEMeSIS and clears all Data Alignment section."

    # Moves to the main interface page
    show_page(pages["Data Reading"])

    # Clean Data Alignment section
    # Classes
    mzML_store.reset()
...
move_to_matrix_reading_button.on_click(_move_to_matrix_reading_button)


# Section of the interface regarding reading the file
data_reading_section = pn.Column(filename_alignment, confirm_button_filename_alignment)
...
def _confirm_button_filename(event):
    ...
    # Change neutral mass column
    if file.neutral_mass_column_inserted:
        if 'Neutral Mass' in file.read_df.columns:
            radiobox_neutral_mass.value = 'Neutral Mass'
        elif 'Probable m/z' in file.read_df.columns:
            radiobox_neutral_mass.value = 'Probable m/z'
        else:
            radiobox_neutral_mass.value = 'None'
    ...
    def __init__(self, **params):
        ...
        widgets = {
            'n_components': pn.widgets.IntInput(name='Number of Components to Compute:',
                                          ...
                                          description='Select 2-20 components. Must be lower than the total number of samples AND features.'),
...
pages = {
    ...
    "Data Alignment": DataAlignmentPage(),
...
page1_button = pn.widgets.Button(name="Data Input", button_type="primary")
...
page1_button.on_click(lambda event: show_page(pages["Data Input"]))
to_data_alignment_button.on_click(lambda event: show_page(pages["Data Alignment"]))
to_matrix_reading_button.on_click(lambda event: show_page(pages["Data Reading"]))
