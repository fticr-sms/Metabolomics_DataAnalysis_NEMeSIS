...
# TODO: Add possiblity of 'Formula_MDiN' checking mult_edge_options to replace 'incoherent' edges instead of only removing the latter.
...
def new_metabolite_info(graph, node, edge, trans_groups, r_type):
    ...
    theo_mass = getmass(new_form) # Theoretical mass of new formula
...
def formula_MDiN(node_list, name_df, trans_groups=None, ppm=1):
    ...
    for extra_edges in set(formula_fail_edges.copy()):
        #print(extra_edges)
    ...
    last_nodes = []
    no_formula_edges_removed = []
    test_1 = edges_to_remove.copy()
    for node1,node2 in edges_to_remove:
        # Only add if one of the nodes has a formula to pass
        if graph.nodes()[node1]['Formula'] != None:
            last_nodes.append(node1)
        if graph.nodes()[node2]['Formula'] != None:
            last_nodes.append(node2)
        if graph.nodes()[node1]['Formula'] == None and graph.nodes()[node2]['Formula'] == None:
            no_formula_edges_removed.append((node1,node2)) # No starting formula in either node, so it will have to be removed
    last_nodes = set(last_nodes) # Nodes in edges to be removed 
    edges_to_remove = [] # Reset and re-obtain the edges_to_remove
    for i in no_formula_edges_removed:
        edges_to_remove.append(i)
    graph, expansion_nodes, edges_to_remove, case4s, formula_fail_edges = formula_attributer(
            last_nodes, graph, trans_groups, expansion_nodes, edges_to_remove, case4s, formula_fail_edges)
    #print(len(set(edges_to_remove)), len(set(test_1)), len(set(edges_to_remove + test_1)))
...
def FDiN_builder(df, filt_elems, trans_groups, col_formula='Formula_Assignment'):
    """Builds a Formula-Difference Network based on the list of formulas provided.
       ...
       filt_elems: pandas DataFrame; DataFrame with the metabolic features as index and the formulas in DataFrame format.
       trans_groups: pandas DataFrame with MDBs in string format as index and the respective formulas in DataFrame format.
       
       return: Networkx Graph (FDiN) object, Transformations are stored as an edge attribute and formulas as node
    attributes."""
    ...
    for formula in tqdm(filt_elems.index):
        ...
        for i in poss_formulas.index:
            poss_matches = filt_elems[(filt_elems == poss_formulas.loc[i]).sum(axis=1) == len(trans_groups.columns)]
            for node in poss_matches.index:
                FDiN.add_edge(formula, node, Transformation=i)
    return FDiN


# Counts GCD-11 orbits
