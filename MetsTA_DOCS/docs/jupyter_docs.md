...
The main jupyter notebook version of NEMeSIS is `NEMeSIS_Data_Analysis_V2.8.ipynb` (currently at Version 2.8). This notebook includes every step of the main steps of data analysis from the alignment of _m/z_ peak lists representing samples to biological interpretation. Furthermore, an extra side module regarding the conversion of raw spectral data in open mzML format to _m/z_ peak lists is available in a separate notebook called `Raw_Spectra_Processing.ipynb` (see here for details [mzML Spectra Conversion](jupyter_docs.md#mzml-spectra-conversion)). This was placed as a separate module to lower memory necessity per individual notebook making the experience smoother. The interactive graphs with a very high number of points in `Raw_Spectra_Processing.ipynb` could affect performance in downstream steps.
...
As for structure, the first few code cells in each case show how the exported data can be read back into the jupyter notebook obtaining all the main data tables (treated data, annotated data, BinSim treated data) as well as the target with the class labels of each sample. They also show the overall details that can be adjusted at the beginning, in these cases, the number of folds to perform stratfied cross-validation and the number of iterations to repeat the cross-validation when fitting and estimating performance of supervised models. These parameters can change based on the type of analysis performed. After this section, each notebook contemplates the necessary procedures to apply their respective methodologies with detailed informations on the different steps present in the notebooks themselves. The `FDiGNN_analysis_module.ipynb` notebook has, at the end, a mini Dash application that will open on the browser to browse and analyse results from the FDiGNN analysis and metabolite importances extracted (as well as pathway analysis) facilitating interpretation.
